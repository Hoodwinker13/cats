...
def choose(paragraphs, select, k):
    ...
    for i in range(len(paragraphs)):
        if(select(paragraphs[i]) and cnt == k):
            return paragraphs[i]
...
def about(topic):
    ...
    def inner(paragraph):
        ...
        for i in range(len(paragraph)):
            if(ord(paragraph[i]) >= 97 and ord(paragraph[i]) <= 122) or ord(paragraph[i]) == 32:
                new_par += paragraph[i]
        print('DEBUG:', new_par)
        for i in topic:
            start = 0
            ...
            while(ind != -1):
                length = len(i)
                ind = new_par.find(i, start)
                start = ind+length
                up = 32
                if(ind+length < len(new_par)):
                    up = ord(new_par[ind+length])
                down = ord(new_par[ind-1])
                if(ind == 0 and up == 32):
                    return True
                elif(ind > 0 and up == 32 and down == 32):
                    return True
        return False
    return inner
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    use = length
    if(length == 0 and len(reference_words) == 0):
        return 100.0
    elif(length==0):
        return 0.0
    if(length > len(reference_words)):
        use = len(reference_words)
    cnt = 0
    for i in range(use):
...
def feline_flips(start, goal, limit):
    ...
    def inner(start, goal, count):
        ...
        if (len(goal) == 0):
            ...
            return count
        if (start[0] == goal[0]):
            return inner(start[1:], goal[1:], count)
        return inner(start[1:], goal[1:], count + 1)

    return inner(start, goal, count )
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    if abs(len(goal) - len(start)) > limit:  # Fill in the condition
        # BEGIN
        return limit + 1
        # END
...
def report_progress(sofar, prompt, user_id, send):
    ...
    for i in range(len(sofar)):
        if(sofar[i] != prompt[i]):
            percent = i/length
            send({'id': user_id, 'progress': percent})
            print(percent)
            return None
    send({'id': user_id, 'progress': len(sofar)/length})
    print(len(sofar)/length)
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        new, prev = 0, 0
        for j in times_per_player[i]:
            new = j
            times[i].append(new - prev)
            prev = j
        times[i].pop(0)
...
def fastest_words(game):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            new = get_times(game)[j][i]
            if (new < min):
                min = new
                min_ind = j
        words[min_ind].append(get_words(game)[i])
    return words
    # END PROBLEM 10
